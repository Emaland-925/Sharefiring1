...
@section('content')
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <h1 class="text-2xl font-bold mb-6">{{ __('messages.edit_user') }}</h1>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input id="email" type="email" class="w-full px-4 py-2 border rounded-md @error('email') border-red-500 @enderror" name="email" value="{{ old('email', $user->email) }}" required>
            ...
            <div class="mb-4">
                ...
                <select id="role" name="role" class="w-full px-4 py-2 border rounded-md @error('role') border-red-500 @enderror" required>
                    <option value="employee" {{ old('role', $user->role) == 'employee' ? 'selected' : '' }}>{{ __('messages.role_employee') }}</option>
                    <option value="company" {{ old('role', $user->role) == 'company' ? 'selected' : '' }}>{{ __('messages.role_company') }}</option>
            ...
            <div class="mb-6">
                <h3 class="text-lg font-semibold mb-2">{{ __('messages.change_password') }}</h3>
                <p class="text-gray-600 text-sm mb-4">{{ __('messages.leave_blank') }}</p>
                
                <div class="mb-4">
                    <label for="password" class="block text-gray-700 mb-2">{{ __('messages.new_password') }}</label>
                    <input id="password" type="password" class="w-full px-4 py-2 border rounded-md @error('password') border-red-500 @enderror" name="password">
                ...
                <div class="mb-4">
                    ...
                    <input id="password_confirmation" type="password" class="w-full px-4 py-2 border rounded-md" name="password_confirmation">
                </div>
            </div>
